...
ssh root@$SQL_NODE "systemctl start mysql"
sleep 3

# Step 5: Verify MySQL connectivity
echo ""
echo "--- Step 5: Verifying MySQL ---"
echo -n "  Node 1 ($MGMT_NODE): "
mysql -u root -phammerdb123 -h $MGMT_NODE -e "SELECT 'OK' as status;" 2>&1 | grep OK || echo "FAILED"

echo -n "  Node 2 ($SQL_NODE): "
mysql -u root -phammerdb123 -h $SQL_NODE -e "SELECT 'OK' as status;" 2>&1 | grep OK || echo "FAILED"

# Step 6: Create root user accessible from both SQL nodes
echo ""
echo "--- Step 6: Setting up root user for remote access ---"
mysql -u root -phammerdb123 -e "
