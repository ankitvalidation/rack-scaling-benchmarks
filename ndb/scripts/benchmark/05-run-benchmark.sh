...
ssh root@$SQL_NODE "cd $HAMMERDB && ./hammerdbcli auto /root/ndb-benchmark/run_benchmark_256vu.tcl" \
...
cd "$HAMMERDB"
./hammerdbcli auto /root/ndb-benchmark/run_benchmark_256vu.tcl 2>&1 | tee "$RESULTS_DIR/node1_result_${TIMESTAMP}.log" &
NODE1_PID=$!
