#!/bin/bash
#===============================================================================
# Redis Cluster 32-inst/node Horizontal Scaling
#
# Uses memtier with --threads=32 --clients=5 --cluster-mode (user's config).
# Single process gets ~31M ops/sec. Use 2-4 processes for saturation.
#
# Test: 3→4→5 nodes × 32 instances/node in cluster mode
#===============================================================================
set +e

MEMTIER="/usr/local/bin/memtier_benchmark"
REDIS_CLI="/usr/local/bin/redis-cli"

SERVERS_3=(200.0.0.103 200.0.0.104 200.0.0.107)
SERVERS_4=(200.0.0.103 200.0.0.104 200.0.0.107 200.0.0.102)
SERVERS_5=(200.0.0.103 200.0.0.104 200.0.0.107 200.0.0.102 200.0.0.101)

PIPELINE=20
DATA_SIZE=256
RATIO="1:1"
TEST_TIME=60
THREADS=32
CLIENTS=5

RESULTS_DIR="/root/redis-scaling-test/results_v11_cluster32"
mkdir -p "$RESULTS_DIR"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
REPORT="${RESULTS_DIR}/report_${TIMESTAMP}.txt"

stop_all() {
    for node in "${SERVERS_5[@]}"; do
        ssh "$node" 'pkill -9 redis-server 2>/dev/null; rm -rf /tmp/redis-cluster' 2>/dev/null &
    done
    pkill -9 redis-server 2>/dev/null; pkill -9 memtier 2>/dev/null
    rm -rf /tmp/redis-cluster 2>/dev/null
    wait; sleep 1
}

start_32_on_node() {
    local NODE=$1
    ssh "$NODE" "
        pkill -9 redis-server 2>/dev/null
        rm -rf /tmp/redis-cluster; mkdir -p /tmp/redis-cluster
        for i in \$(seq 0 31); do
            P=\$((7000+i))
            mkdir -p /tmp/redis-cluster/\$P
            redis-server --port \$P --bind 0.0.0.0 --protected-mode no \
                --dir /tmp/redis-cluster/\$P \
                --cluster-enabled yes --cluster-config-file /tmp/redis-cluster/\$P/nodes.conf \
                --cluster-node-timeout 5000 \
                --appendonly no --save '' --maxclients 200000 \
                --tcp-backlog 65535 --hz 100 --daemonize yes 2>/dev/null
        done
        echo \"\$(pgrep -c redis-server) on \$(hostname -s)\"
    " 2>&1 | grep " on "
}

start_32_local() {
    pkill -9 redis-server 2>/dev/null
    rm -rf /tmp/redis-cluster; mkdir -p /tmp/redis-cluster
    for i in $(seq 0 31); do
        P=$((7000+i))
        mkdir -p /tmp/redis-cluster/$P
        redis-server --port $P --bind 0.0.0.0 --protected-mode no \
            --dir /tmp/redis-cluster/$P \
            --cluster-enabled yes --cluster-config-file /tmp/redis-cluster/$P/nodes.conf \
            --cluster-node-timeout 5000 \
            --appendonly no --save '' --maxclients 200000 \
            --tcp-backlog 65535 --hz 100 --daemonize yes 2>/dev/null
    done
    echo "  $(pgrep -c redis-server) on $(hostname -s)"
}

create_cluster() {
    local NODES=("$@")
    local CLUSTER_ARGS=""
    for node in "${NODES[@]}"; do
        for p in $(seq 0 31); do
            CLUSTER_ARGS="${CLUSTER_ARGS} ${node}:$((7000+p))"
        done
    done
    local TOTAL=$(echo $CLUSTER_ARGS | wc -w)
    echo "  Creating ${TOTAL}-node cluster..."
    $REDIS_CLI --cluster create $CLUSTER_ARGS --cluster-replicas 0 --cluster-yes 2>&1 | tail -3
    sleep 5
    echo "  $($REDIS_CLI -h ${NODES[0]} -p 7000 cluster info 2>/dev/null | grep cluster_state)"
    echo "  $($REDIS_CLI -h ${NODES[0]} -p 7000 cluster info 2>/dev/null | grep cluster_known_nodes)"
}

run_benchmark() {
    local LABEL=$1
    local ENTRY=$2
    local NUM_NODES=$3
    local NUM_PROCS=$4

    local PIDS=()
    echo "  Running: ${NUM_PROCS} procs × ${THREADS}T × ${CLIENTS}C | pipeline=${PIPELINE}"

    for p in $(seq 1 $NUM_PROCS); do
        $MEMTIER --server="$ENTRY" --port=7000 \
            --protocol=redis --cluster-mode \
            --threads=$THREADS --clients=$CLIENTS --pipeline=$PIPELINE \
            --data-size=$DATA_SIZE --ratio=$RATIO --test-time=$TEST_TIME \
            --key-minimum=1 --key-maximum=10000000 --key-pattern=R:R \
            --hide-histogram > "${RESULTS_DIR}/${LABEL}_p${p}_${TIMESTAMP}.txt" 2>/dev/null &
        PIDS+=($!)
    done

    echo "  Waiting for ${#PIDS[@]} procs (${TEST_TIME}s + cluster discovery)..."
    for pid in "${PIDS[@]}"; do wait "$pid" 2>/dev/null || true; done

    # Aggregate
    local TOTAL_OPS=0 TOTAL_P99=0 COUNT=0
    for f in "${RESULTS_DIR}/${LABEL}_p"*"_${TIMESTAMP}.txt"; do
        local OPS=$(grep "^Totals" "$f" 2>/dev/null | awk '{print $2}')
        local P99=$(grep "^Totals" "$f" 2>/dev/null | awk '{print $7}')
        if [[ -n "$OPS" && "$OPS" != "0.00" ]]; then
            TOTAL_OPS=$(echo "$TOTAL_OPS + $OPS" | bc)
            TOTAL_P99=$(echo "$TOTAL_P99 + $P99" | bc 2>/dev/null || echo "$TOTAL_P99")
            COUNT=$((COUNT+1))
        fi
    done

    if [[ $COUNT -gt 0 ]]; then
        local AVG_P99=$(echo "scale=2; $TOTAL_P99 / $COUNT" | bc)
        local OPS_INT=$(printf "%.0f" "$TOTAL_OPS")
        local PER_NODE=$((OPS_INT / NUM_NODES))
        echo "  >>> ${OPS_INT} ops/sec | p99: ${AVG_P99} ms | per-node: ${PER_NODE} [${COUNT}/${NUM_PROCS} procs]"
        echo "${LABEL} | ${OPS_INT} | p99: ${AVG_P99} ms | ${PER_NODE}/node | ${COUNT}/${NUM_PROCS} procs" >> "$REPORT"
    else
        echo "  >>> ERROR: no valid results"
        echo "${LABEL} | ERROR" >> "$REPORT"
    fi
}

#===============================================================================
echo "==============================================================================="
echo " Redis Cluster Horizontal Scaling — 32 instances/node"
echo " $(date)"
echo " memtier config: threads=${THREADS}, clients=${CLIENTS}, pipeline=${PIPELINE}"
echo " Entry point: port 7000 on first server, cluster-mode auto-discovers all"
echo "==============================================================================="

{
echo "Redis Cluster 32-inst/node Scaling — $(date)"
echo "Config: threads=${THREADS}, clients=${CLIENTS}, pipeline=${PIPELINE}, data=${DATA_SIZE}B"
echo ""
} > "$REPORT"

# --- 3 nodes × 32 = 96 cluster members ---
echo ""
echo "--- 3 nodes × 32 instances = 96 cluster members ---"
stop_all
for node in "${SERVERS_3[@]}"; do start_32_on_node "$node"; done
sleep 3
create_cluster "${SERVERS_3[@]}"
sleep 5
run_benchmark "cluster_3n_32i" "200.0.0.103" 3 3

# --- 4 nodes × 32 = 128 cluster members ---
echo ""
echo "--- 4 nodes × 32 instances = 128 cluster members ---"
stop_all
for node in "${SERVERS_4[@]}"; do start_32_on_node "$node"; done
sleep 3
create_cluster "${SERVERS_4[@]}"
sleep 5
run_benchmark "cluster_4n_32i" "200.0.0.103" 4 3

# --- 5 nodes × 32 = 160 cluster members ---
echo ""
echo "--- 5 nodes × 32 instances = 160 cluster members ---"
stop_all
for node in "${SERVERS_4[@]}"; do start_32_on_node "$node"; done
start_32_local
sleep 3
create_cluster "${SERVERS_5[@]}"
sleep 5
run_benchmark "cluster_5n_32i" "200.0.0.103" 5 3

#-------------------------------------------------------------------------------
stop_all
echo ""
echo "==============================================================================="
echo " COMPLETE — $(date)"
echo "==============================================================================="
echo ""
cat "$REPORT"
